...
| --- | --- | --- | --- | --- |
| Expert-cache slots | 8, 16, 24, 32, 64, 96, 128; CLI also accepts resident and auto | `--expert-cache-slots` | CLI/server auto | Auto always uses the slot cache: Qwen gets 96 slots on hosts with at least 24 GiB, 32 with at least 16 GiB, and 16 otherwise; other families get 16. `resident` maps every layer file once and skips the slot cache entirely — it lost the community A/B to the slot rungs (page-cache thrash), so it remains an explicit opt-in. More slots retain more routed experts and reduce later reads at the cost of RAM. |
