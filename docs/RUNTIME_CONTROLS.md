...
| Prompt prefill | On, off | — | On | Every shipped family selects a native chunked production path. Flash-Next and the other non-DeepSeek families do not replay the prompt token by token on the host; model-specific recurrent scans still advance in token order inside a GPU kernel where the math requires it. DeepSeek retains its documented long-span fallback. Off selects correctness-first scalar replay for internal parity and diagnostics rather than skipping prompt processing. |
