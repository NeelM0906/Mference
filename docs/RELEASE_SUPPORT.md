...
Status reviewed September 22, 2026; PRs #37 and #38 are merged. The 0.1.0 source
...
| `glm53flash` | Advanced text option with explicitly qualified profiles | 180.84 GB | Pinned install and resident/16-slot sparse-cutover, continuation and cancellation/reset correctness gates pass. [Max/1,024 warmups](RELEASE_PERFORMANCE_2026-09-20.md#glm-default-profile-warmups-rejected-completion-gate-safety-stop) truncate without visible answers. The [matched low-effort resident profile](POSTLAUNCH_QUALIFICATION.md#matched-glm-baseline) completes all 24 processes with identical answers: long generation improves 3.7%, short/medium ranges overlap, no decode gain. September 22 [Max/8,192 resident](RELEASE_VALIDATION_2026-09-22.md#glm-max-profile) and [low-effort 16-slot](RELEASE_VALIDATION_2026-09-22.md#glm-bounded-profile) profiles each complete 9/9 measured answers. Bounded mode is slower on this host. All new evidence is from 256 GiB; reduced slots do not certify a smaller Mac. [Settings and limits](families/GLM53_FLASH.md). |
...
Flash-Next native MTP's independent 43-row installed numerical comparison now
passes, along with its internal exact-target verification/recovery tests.
Accelerated CLI/server generation remains disabled; these results do not
qualify native speed/default promotion. See the
[native MTP boundary](FLASHNEXT_MTP_STATUS.md).
