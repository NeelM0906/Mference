...
Local receipt totals were inspected September 18; MiniCPM/GLM use the linked
...
| `swiftqwen38` | Optional candidate, not the recommended replacement yet | 15.38 GB | Numerical, state and tool/UI gates on 256 GiB M3 Ultra; updated broader task/latency screen pending. No new 24 GB qualification. Distinct model ID and [source license](families/SWIFT_QWEN38.md). |
...
| `glm53flash` | Advanced qualification candidate for this update | ~180.8 GB planned | Historical family gate; current install and real validation of the new prefill kernels pending. Resident target is the 256 GiB host; streamed synthetic tests do not certify a smaller Mac. [Evidence](families/GLM53_FLASH.md). |
...
| `inklingsmall` | Retained specialist / large MoE | 148.42 GB | Historical 24 GB M5 measurements; current short regression on 256 GiB M3 Ultra. Wider boundary/cancellation matrix remains incomplete. [Evidence](INKLING_SMALL.md). |
...
existing MTP policy is unchanged. Base Qwen, Maple, MiniCPM and GLM use their
checkpoint-specific thinking templates rather than Swift's effort parameter;
do not carry an explicit Swift effort setting into another model's UI chat.
