...
| `qwen38flashnext` | Advanced flagship MoE | 175.21 GB | INT8-router profile: 4,096-context evidence on 256 GB M3 Ultra; resident/16-slot short and 1,024-chunk sparse-boundary correctness gates pass with observed TensorOps execution. [September 20 resident measurements](RELEASE_PERFORMANCE_2026-09-20.md) at `15713e7` reduce prefill-plus-decode time but regress short/medium prefill; streamed comparison remains open. Packaged MTP weights do **not** imply working native MTP decoding. [Family evidence](families/QWEN38_FLASH_NEXT.md). |
