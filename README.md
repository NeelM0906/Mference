...
| Storage | ~14.3 GB installed (Gemma 4) · ~14.45 GB (Gemma 4 QAT; 14,451,052,105 bytes) · ~19.6 GB (Qwen 3.6) · ~91 GB (DeepSeek-V4-Flash) · ~148 GB (Inkling-Small) · ~6.6 GB (Maple) · ~15 GB (Qwen 3.8) · ~175 GB (Flash-Next) · 1.43 GB (MiniCPM5-2B; 1,425,981,882 bytes over 8 files) |
...
| Platform | macOS 15+, Metal 3 (MSL 3.2), Swift 6.1+; running on macOS 26 adds the Metal 4 tensor-ops prefill kernels wherever their pipelines build (some require an Apple10 GPU) |
...
therefore covers both worlds: *running* on macOS 26 compiles at MSL 4.0 and
enables the Metal 4 tensor-ops prefill kernels wherever their pipelines build
(some require an Apple10 GPU), while macOS 15 compiles at MSL 3.2 and selects
the non-tensor kernels automatically. Both paths run the full feature set; they
