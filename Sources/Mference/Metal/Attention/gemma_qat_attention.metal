...
kernel void gemma_qat_attention_swa(
    ...
    constant uint& end [[buffer(4)]], constant uint& start [[buffer(5)]],
    constant uint& ringCapacity [[buffer(6)]],
    ...
    for (uint position = start + group; position < end; position += 32u) {
        ...
        const uint base = (slot * 8u + kvHead) * 256u + lane * 8u;
        float score = 0.0f;
...
kernel void gemma_qat_attention_scores(
    device const half* query [[buffer(0)]], device const half* keys [[buffer(1)]],
...
kernel void gemma_qat_attention_values(
