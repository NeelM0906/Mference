...
// The same grouping for Qwen 3.6's 256/16/2 full attention. At head_dim 256
// the per-head kernel gives each of its 256 threads one component, sums each
// SIMD group, then sums the eight group totals in SIMD-group order; lane l of
// query head h here holds components l + 32g, so every sum sees the same values
// in the same lanes and the output is bit-identical.
[[kernel, max_total_threads_per_threadgroup(kAttnThreads)]]
void attention_decode_full_grouped_vec256_partial(
    device const half*  Q             [[buffer(0)]],
    device const half*  K             [[buffer(1)]],
    device const half*  V             [[buffer(2)]],
    device       float* m_out         [[buffer(3)]],   // [num_q_heads * num_chunks]
    ...
    constexpr uint kGVHeadDim  = 256;
    constexpr uint kGVQPerKV   = 8;
    constexpr uint kGVPerLane  = kGVHeadDim / 32;   // 8 components per lane
    ...
    for (uint p = p_start; p < p_end; ++p) {
        ...
        float group_total = 0.0f;
        for (uint g = 0; g < kGVPerLane; ++g) {
            const float partial = fma(q_reg[g], float(K_row[32 * g]), 0.0f);
            // Every lane participates before one lane retains the virtual sum.
            const float reduced = simd_sum(partial);
            if (simd_lane_id == g) { group_total = reduced; }
        }
